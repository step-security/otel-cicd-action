...
| --------------- | ----------------------------------------------------------------------------------------------------------- | -------- | ------------------------------------- | ---------------------------------------------------------------- |
| otlpEndpoint    | The destination endpoint to export OpenTelemetry traces to. It supports `https://`, `http://` and `grpc://` endpoints. **Avoid `http://` with real credentials — use it only for local collectors (e.g. `http://localhost:4318`), as headers are transmitted in cleartext.** | true     |                                       | `https://api.axiom.co/v1/traces`                                 |
| otlpHeaders     | Headers to add to the OpenTelemetry exporter. Typically contains API tokens or auth credentials. Always pair with an `https://` or `grpc://` endpoint to ensure credentials are encrypted in transit. | true     |                                       | `x-honeycomb-team=YOUR_API_KEY,x-honeycomb-dataset=YOUR_DATASET` |
